...
if binning==0 and cross_val==1:
    ...
    start_ts=time.time()
    rgr=AdaBoostRegressor(n_estimators=100, loss='linear', learning_rate=0.5, base_estimator=None, random_state=rand_st)
    scores=cross_validate(rgr, data_np, target_np, scoring=scorers, cv=5)                                                                                                   
    ...
    scores_RMSE = np.asarray([math.sqrt(-x) for x in scores['test_Neg_MSE']])                                       #Turns negative MSE scores into RMSE
    scores_Expl_Var = scores['test_expl_var']
    print("MLP Regressor RMSE:: %0.2f (+/- %0.2f)" % ((scores_RMSE.mean()), (scores_RMSE.std() * 2)))
    print("MLP Regressor Expl Var: %0.2f (+/- %0.2f)" % ((scores_Expl_Var.mean()), (scores_Expl_Var.std() * 2)))
    print("CV Runtime:", time.time()-start_ts)
