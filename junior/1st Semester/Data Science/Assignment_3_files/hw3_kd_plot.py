...
def plot_clusters(DB, labels, file_name):
    clusters = {}
    ...
    for cluster_id, obj_ids in clusters.items():
        cluster_points = np.array([DB[obj_id] for obj_id in obj_ids])
...
if __name__ == "__main__":
    ...
    plot_clusters(db, labels, f"dbscan_clusters_{input_file.split('/')[-1].split('.')[0][-1]}.png")
